...
**全部 6 个既有实验（E1/E2/E3/A1/A2/A3）的 harness 已在验证机 0.5B 跑通并自检 PASS**（A3 与 E2 共享数据）。D-016 的 E1/E2 确认性 campaign 当前为“代码已实现并与最终 CLI 对齐，GPU 待执行”，不与既有旧 runner 状态混淆。完整状态表 + 实验机待办清单见 `paper2/experiment_design.md` §9'。
...
**下一个里程碑（D-016 后）**：`e1e2_confirmatory` 代码与最终 CLI 已对齐；GPU 操作者按 `GPU_HANDOFF.md` 从 clean commit 依次执行 disjoint holdout、TEN cache、不可变 campaign manifest、`--limit 3` 非 formal pilot、5 个强制共享 manifest 的独立 formal 进程（session-index 0..4）、validation/analysis、hash/tarball 和 acceptance。直接审计 raw `last_segment_arrival→first_token_ready` 与 validator 恒等式；不得用 `endpoint_accept` 或 `TTFT_eff` 替代。正式结果验收前不得修改论文数字；验收后才决定是否统一替换权威 Markdown 的 E1/E2 数值并重画图。P1 v2、固定轨迹 E3 与联合 A1 不再重跑。并行后续仍包括更接近生产的真实流式 ASR/在线 TTS 取消/音频队列/声卡扬声器闭环、固定轨迹 A2，以及随机盲法双标注人评；这些真实音频与人评工作不由 D-016 的受控文本段 campaign 替代。
