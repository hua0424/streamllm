...
**下一个里程碑（D-017 后）**：确认性 E1/E2 已验收并入正文；论文进入导师审阅与格式/引用定稿（GB/T 7714、访问日期、数据可用性声明），IEEE 中英衍生稿待权威 Markdown 稳定后整体同步。不再重跑 P1 v2、固定轨迹 E3、联合 A1 或确认性 E1/E2。并行后续仍包括更接近生产的真实流式 ASR/在线 TTS 取消/音频队列/声卡扬声器闭环（含"真实端点晚于触发时推测收益"的验证）、固定轨迹 A2，以及随机盲法双标注人评；这些真实音频与人评工作不由受控文本段 campaign 替代。
