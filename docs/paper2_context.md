...
**实验机当前唯一无条件待办**：按 `experiments/sci34_supplement/c2_crop_integrity/GPU_HANDOFF.md` 执行 Qwen2-7B C2 v3 exact-only crop-integrity addendum。C2 v1/v2 均已执行并 rejected 归档；v3 不修改两轮 verdict、不重跑 termination，而直接验证 production crop retained K/V 与独立切片 oracle bitwise exact。E3 rescue 与既有主 LLM/TEN/CosyVoice/judge campaign 只读，不再执行。

**环境现状**：v3 compile/fake full workflow 通过（24/27 与全部 tamper）；本机 0.5B CUDA 真模型对 c2_01/c2_08 的 3 个真实 crop（含 next-turn/second crop）验证 pre-prefix=production-post=oracle、matched recovery K/V/logits/state 全 bitwise/exact。正式证据仍须目标 Qwen2-7B snapshot、BF16/backend 和 exact clean commit。
...
**下一个里程碑（D-021 后）**：完成并验收 Qwen2-7B C2 v3 crop-integrity formal；结果回传后统一处理 crossed analysis、E3 weighting/sensitivity、事件命名、贡献层级和论文正文。论文只能按 v3 边界主张 crop/truncation integrity 与 matched recovery determinism，不得称 clean re-prefill 数值等价或 v2 通过。C2 v1/v2、P1 v2、固定轨迹 E3、联合 A1、确认性 E1/E2 均不重跑。
