...
| 日期 | 里程碑 | 产出 |
|---|---|---|
| 2026-09-08 | **R/B JSON validator 修复（D-028）** | e82551b 报告旧 pilot 8 records 后 cases tuple/list 断言失败，formal 未启动；真实 manifest 读边界先红后绿，最小 JSON 归一化；完整 CPU 合成 8/320 records 与 case 篡改负控、R/B/C2 v2/v3 smoke 通过。旧失败只读，本机无归档可重放；未 commit/push、未跑 GPU，须新 clean 修复 commit 后新 pilot 成功再 formal。 |
