...
| 日期 | 里程碑 | 产出 |
|---|---|---|
| 2026-09-08 | **R/B 正式接受与 SC 单主线改稿完成（D-029）** | formal `rb_formal_20260908T023934Z_f69e6478`：320 records/160 event pairs，4 sessions，B 100 trajectories/1118 cursors/27 closures；完整 tar 前轮已核验。SC 以 joint prefix-state repair 为唯一方法，新增 R 全 cell 成本表，C1/C-E1/C-E2/A2 实际移入独立编译 supplement，保留 C2 failures 与 inconclusive E3。无新实验、无 GPU 待办；最终主稿 27 页/补充 8 页；只读科学代理 `6343...` 完成 30 R CI/E3 PASS，最终 judge 关闭全部可修复排版缺陷（非独立 panel）。无科学/视觉排版待办；主稿 1/23/24/25、补充 1 作者事实/声明及权利仍阻塞投稿，未假造解决；最终文档包检查见 `review/sc_revision_validation_2026-09-08/final_status/`。详见 dated revision record；不改中文权威章节。 |
| 2026-09-08 | **R/B JSON validator 修复（D-028，历史阶段；后续已由 D-029 完成）** | e82551b 报告旧 pilot 8 records 后 cases tuple/list 断言失败，formal 未启动；真实 manifest 读边界先红后绿，最小 JSON 归一化；完整 CPU 合成 8/320 records 与 case 篡改负控、R/B/C2 v2/v3 smoke 通过。旧失败只读，本机无归档可重放；未 commit/push、未跑 GPU，须新 clean 修复 commit 后新 pilot 成功再 formal。 |
...
| 2026-09-03 | **C2 v2 formal rejected；exact-only v3 addendum 冻结（D-021）** | v2 run `c2eq_5c56b014_20260903T040829Z`：24/24 probe、45/45 token/state/EOT/scenario、top-k/近并列规则全过，但单控制 2× raw-logit 门槛 42/45，按预注册规则 rejected。独立审计确认 control 与 production forward 拓扑不匹配；三失败由尾部词表极值或 softmax 不变常数偏移主导，不能归因 crop，也不允许事后放宽倍数。新增 `c2_crop_integrity/` v3：24 cases/27 events，assistant 逐 token 走真实生成追加；production crop 与同 pre-crop K/V 独立切片 oracle 全层 bitwise/hash exact，matched recovery K/V/logits/mask/ledger/role 状态 exact；validator 独立推 keep，wrong-length negative control。fake full workflow 与 0.5B CUDA 3-event 真模型 pilot 全绿；7B formal pending。 |
| 2026-09-03 | **C2 v3 pilot 暴露并修复陈旧 CROPPED 状态（D-022）** | 7B v3 pilot（`91ea218`）7/8 cases 与全部 K/V exact 门通过；唯一失败为 invalidation crop 后直接 `prefill_user_text` 时 generation_end_reason 仍为 CROPPED。判定为生产状态推进缺陷：CROPPED 在 crop 后可见，但新 user 内容成功追加后应清 NONE。`prefill_user_text` 补一行清零，orchestrator 加 fail-closed；真实 run_kvcrop 回归红→绿，0.5B c2_06 K/V/logits/state exact。既有实验数值不受影响，不重跑；v3 协议不变。 |
| 2026-09-03 | **C2 v3 formal 全部通过并正式接受（D-023）** | run `c2crop_82103004_20260903T080512Z`（code `8210300`、result `7d50624`、manifest `d8c3db4d…`）：24/24 cases、27/27 crop events、3 no-op、60 recovery steps 全 exact；308 fixture tokens 逐 token production append，28 层 retained K/V 的 pre-prefix/production-post/independent-oracle 三方 bitwise/hash 一致，matched recovery K/V/logits/mask/ledger/role-end state exact，27/27 wrong-length 负控检出。validation 零错误、analysis accepted、ACCEPTANCE accepted、30-file seal `e0997d41…` 验证通过，381-file legacy guard 一致。设计侧在 LF 保留 clone 中重跑 validator/seal 全绿。GPU 阻塞解除，进入论文统一修订；v1/v2 仍保持 rejected 描述性证据。 |
...
**实验机当前无无条件待办**：Qwen2-7B C2 v3 exact-only crop-integrity 已按 D-023 接受并封存；C2 v1/v2 rejected、v3 accepted、E3 rescue 与既有主 LLM/TEN/CosyVoice/judge campaign 全部只读，不再执行。
