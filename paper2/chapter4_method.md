...
- **推测阈值 $\theta$**（激进）：$c_i \ge \theta$ 即启动**推测生成**——注入 assistant 角色标记并预生成至多 $\kappa$ 个 token（$\kappa$ 为推测预算，限制单次作废成本）；
...
3. **评估** $c_i$；$c_i \ge \theta$ → 记录推测起点 $b := |\mathcal{K}|$，注入 assistant 标记，预生成 $\le \kappa$ token 并缓存。
