...
- 双 3090 模型布局
- 组件检查与数据级检查的证据边界
- 固定轨迹 E3/联合 A1 已补 manifest、数据/模型指纹与原始重复；旧 campaign 仍缺部分元数据
...
- 不是完整 barge-in；不跨 CPU campaign 池化绝对时间
...
- 外部效度：单模型、双 3090、英文任务型对话、跨 CPU campaign 不池化
- 结论效度：cluster bootstrap 主、McNemar 描述；不显著不等于等效/非劣，负点估计不等于伤害
...
- C2 的状态机制与 A1 计算结果成立；固定轨迹 E3 未检出预设方向语义收益；C1 为受控模拟，C3 为受混杂的探索性负结果

### 8.2 后续工作
- 真实异步音频闭环与协议修订后的 P1 v2
