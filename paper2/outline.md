...
### 5.5 部署、验证与可复现性
- 双 3090 模型布局
- 三个独立 campaign：旧 E1/E2/A2、固定轨迹 E3/联合 A1、prepared-state P1 v2；绝对时间不池化，A1/P1 不相减
- P1 主机：双路 Xeon Gold 6330、112 逻辑 CPU、约 756 GiB、Ubuntu 22.04.5、driver 580.105.08、双 RTX 3090
- 组件检查与数据级检查的证据边界
- 固定轨迹 E3/联合 A1/P1 v2 已归档 manifest、模型/运行身份与原始重复；旧 campaign 仍缺部分元数据
...
### 6.1 研究问题与实验设置
- RQ1 一致性；RQ2 推测权衡；RQ3 组合响应；RQ4 KV 复用与 prepared-state 软件打断控制路径；RQ5 历史策略
- E2 清除 fixture；固定轨迹 E3 独立使用 100 条纯 MultiWOZ，不与旧 E3 合并
...
#### 6.5.1 A1：KV 复用联合微基准
- 256–8192 token；warmup=5，repeats=50；设备同步的同一区间联合计时
...
- 紧凑表 6-5：stop ack 0.055–0.062 / 最大单元 P95≈0.077；post-stop sync 0.167–0.176 / ≈0.352；lookup 0.47–0.50 / ≈0.94；stop→crop 2.44–2.53 / ≈3.492；stop→role 78.6–80.8 / ≈86.1 ms
...
- 内部效度：同步时序、40-token 上限、0.5/boundary 重复、A2 条件轨迹不一致
- 外部效度：单模型、双 3090、英文任务型对话、三个 campaign 不池化
- 结论效度：cluster bootstrap 主、McNemar 描述；不显著不等于等效/非劣，负点估计不等于伤害；P1 v1 协议失败只作审计，v2 当前有效但限于软件控制路径
...
- C2 的状态机制、A1 模型侧计算结果和 P1 prepared-state 软件控制路径结果成立；固定轨迹 E3 未检出预设方向语义收益；C1 为受控模拟，C3 为受混杂的探索性负结果
