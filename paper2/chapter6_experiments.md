...
**检测器方法学**（单列一段，兼作 7.x 节 threats 的依据）　规则检测器与 LLM 裁判在真实数据上一致性很低（Cohen's κ≈0.05；开发用小样本上为 0.71）。原因是 MultiWOZ 任务域高频词（hotel、book、train 等）使词面线索在域内自然复现，规则检测器系统性过触发。因此本文将规则口径解释为**表面重叠上界**、裁判口径为**特定引用下界**，真值介于其间；关键在于**两把尺子下 B-ours 均不劣于 B-gen，且 loose 口径双双为零**，主结论对检测器选择鲁棒。[占位：37 条人工校验样本（κ_人-规则 / κ_人-裁判）完成后补 2–3 句与小表]
