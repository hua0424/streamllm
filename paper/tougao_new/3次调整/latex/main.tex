% 由 Word 终稿转换（Pandoc 3.10 + 人工整理）。原始 Pandoc 输出见 pandoc_raw.tex。
% 编译方式：xelatex main.tex（需编译两遍以生成交叉引用）
% 说明：
%  - 原 Word 中的 MathType/OLE 公式对象已根据其内嵌文本重建为 LaTeX 数学公式。
%  - 六张矢量图已由 SVG 转为 PDF（media/media/imageNN.pdf）。
%  - 原文 3.3.2 节引用了 "Algorithm 1"，但 Word 终稿中不存在对应的算法框，此处保留原引用。
...
\begin{center}
{\Large\bfseries 流水线并行流式架构的级联式语音对话系统延迟优化}
\end{center}

\noindent\textbf{摘\quad 要：}针对级联式语音对话系统在长语音场景下端点后等待时间随输入时长增长的问题，提出流水线并行的流式优化架构，使语音识别、语言模型推理可重叠执行。前端以语音活动检测实时分段，结合 Whisper 时间戳对齐构建自适应滑窗与动态缓冲，通过前缀-后缀上下文与局部一致性约束提交稳定转录；后端采用键值缓存增量预填充，仅对新增文本更新缓存，降低首token输出时间并限制其不随长度线性增长。在由 MultiWOZ（英文）与 CrossWOZ（中文）合成的 1132 条递增长度语音测试集上，长语音分组的平均首 token 输出时间稳定在约 1.1 秒，相比非流式基线降低 34.6\%--83.9\%，最长分组平均减少 5.67 秒且转录错误率保持在可接受范围。结果表明该架构可有效降低长语音交互等待。

\noindent\textbf{关键词}：流式架构；语音对话系统；流水线并行；增量预填充；端到端延迟

\noindent 文献标志码: A \quad 中图分类号: TP18
...
\noindent\textbf{Key words}: streaming architecture; voice dialogue system; pipeline parallelism; incremental prefilling; end-to-end latency
...
相比之下，新一代语音对话系统基于大语言模型有更强的理解能力，能处理复杂推理任务，还能支持带有情感色彩的多轮自然语言交互，可这种智能化水平的提高，也带来了巨大的计算开销和延迟挑战。公开的系统报告指出，GPT-4o端到端语音响应延迟可低至232毫秒，平均320毫秒\upcite{ref1}，这已经接近人类自然对话反应速度。这意味着，低延迟与准确率并列，是下一代语音交互体验的关键竞争指标。

尽管端到端多模态模型在论文和实验中表现出色，可其训练成本高，数据隐私管控难，垂直场景需深度定制。而级联式系统是把语音活动检测（Voice Activity Detection, VAD）\upcite{ref2}、自动语音识别、大语言模型和语音合成拆分为独立可替换的模块并按顺序串联，因此可以复用现有的高质量ASR、文本LLM与TTS能力。在需要快速迭代LLM、控制部署成本、降低升级风险的场景中，级联架构仍然具备重要的工程价值。在这种串行架构中，系统的总延迟（$T_{\mathrm{total}}$）不仅取决于各个模块的处理时间，还显著受限于模块间的数据流转方式。为方便分析，这种串行级联系统的总延迟可表示为各组件延迟的简单线性叠加：
\begin{equation}
T_{\mathrm{total}} = T_{\mathrm{VAD}} + T_{\mathrm{ASR}} + T_{\mathrm{Prefill}} + T_{\mathrm{Decode}} + T_{\mathrm{TTS}} + T_{\mathrm{Net}}
\end{equation}

其中，$T_{\mathrm{VAD}}$表示语音活动检测及端点判定延迟，$T_{\mathrm{ASR}}$表示语音识别延迟，$T_{\mathrm{Prefill}}$表示 LLM 对输入上下文进行预填充计算的延迟，$T_{\mathrm{Decode}}$表示 LLM 自回归生成阶段的解码延迟，$T_{\mathrm{TTS}}$表示语音合成延迟，$T_{\mathrm{Net}}$表示网络传输、请求调度及数据序列化等通信开销。需要说明的是，本文的首字延迟（Time to First Token, TTFT）主要用于衡量系统从用户语音输入结束到 LLM 生成首token 的响应时间，因此统计口径包含$T_{\mathrm{VAD}}$、$T_{\mathrm{ASR}}$、$T_{\mathrm{Prefill}}$以及生成首token 所需的初始解码延迟，不包含$T_{\mathrm{TTS}}$和$T_{\mathrm{Net}}$。
...
\subsection{自动语音识别的流式化进展}

ASR系统的处理速度，决定着用户语音转化为文本LLM可接受输入的速度。早期流式自动语音识别的架构演进以RNN-Transducer（RNN-T）架构为主导\upcite{ref3}。RNN-T 凭借循环神经网络的天然时序特性，非常适合处理流式音频，但其逐时刻的循环依赖使训练时难以在时间维度上充分并行，并行训练效率受限，且长程语义捕捉较为困难。随着 Transformer的出现，注意力机制模型的识别精度得以提升到新高\upcite{ref4}。但是Transformer的全局注意力要求必须在整段输入完成后才开始计算，因此原生Transformer结构无法支持流式运行。

为了解决这个问题，Google提出了Conformer架构，它把卷积神经网络（Convolutional Neural Network, CNN）的局部特征提取和Transformer全局建模结合起来，很快就成了业界主流\upcite{ref5}。在实际应用中，流式ASR常采用分块处理再加上重叠拼接的方式：先用滑动窗口切分音频并识别，然后通过重叠区域对输出做一致性校正，从而在准确率与延迟之间达成平衡。以Whisper为例，它的原始模型是采用Encoder-Decoder结构的，不过主要面向离线转录\upcite{ref6}；围绕此类离线模型的流式化，已有工作或从模型结构入手，将非因果编码器改造为因果编码器，使其能在更小的分片上实现较低延迟的在线转录，或从训练目标入手，借助序列级正则化激励token更早地发射，以缩短流式识别的输出滞后时长。

\subsection{大语言模型推理加速技术}

在ASR完成转录之后，LLM的推理速度就成了系统延迟的第二大瓶颈。这一过程主要受Transformer解码器的自回归生成机制以及GPU的显存带宽所限\upcite{ref7}。键值缓存（Key-Value Cache, KV Cache）技术的出现正是为了解决推理过程里的重复计算问题。该技术通过缓存历史token 的Key/Value向量值，这样在生成新token时，就不需要对历史上下文重复计算中间结果了，由此可以将解码阶段的单步计算复杂度从与序列长度二次相关降低为线性相关。现在KV Cache已成为vLLM、TGI等现代推理引擎的标配\upcite{ref8}。在长文本输入的预填充（Prefill）阶段，FlashAttention改进了GPU显存的读写方式，采用分块计算策略，让长序列的吞吐量提高到新的高度，从而在一定程度上解决注意力计算量随序列长度迅速增大这一问题\upcite{ref9}。在长对话场景下，StreamingLLM引入“注意力汇聚”机制，把早期token作为固定锚点，以缓解上下文窗口溢出带来的性能下降，使LLM在有限的KV Cache里还能持续稳定地流式输出。

\subsection{语音对话系统的全链路优化}

除了单独优化ASR或者LLM之外，全链路协同调度对减少整体延迟也很重要。在语音翻译等语音处理任务中，级联方案长期作为常见系统范式存在，其优势在于可复用现有成熟组件、模块替换灵活，但也会带来中间表示损失、误差传播以及模块间延迟累积等问题\upcite{ref10}。近期端到端流式语音对话模型尝试绕过传统ASR-LLM-TTS串行链路，通过语音原生建模和流式生成追求更低的交互延迟\upcite{ref11}。Mini-Omni、Moshi和LLaMA-Omni等开源系统即属于此类探索。
...
\caption*{\textbf{图1\quad Whisper 模型整体架构与特征降采样示意图}\\
Fig.~1\quad Overall architecture of the Whisper model and schematic of feature downsampling}
\end{figure}
...
\subsubsection{Whisper 模型架构与离线特性}

本研究采用OpenAI的Whisper作为语音转文本模块。Whisper是基于Transformer的Encoder-Decoder结构的端到端语音转文本模型，它直接根据语音特征输出文本token。该模型在大规模弱监督语音-文本数据上预训练，在多语言、口音和噪声场景下具有较强鲁棒性\upcite{ref12}。
...
\subsection{大语言模型推理机制}

\subsubsection{Transformer 注意力机制}
...
% 注：下段中的 W_Q/W_K/W_V、d_k、QK^T 三处符号在 Word 原稿中为嵌入对象且未随导出保留，
% 已按缩放点积注意力的标准记法补齐。
其中，$Q$(Query)、$K$(Key)、$V$(Value) 分别由输入向量经过线性投影矩阵 $W_Q$、$W_K$、$W_V$ 得到。$d_k$ 为 Key 的维度，用于缩放点积的数值范围，避免 $QK^{\top}$ 随维度增大而方差膨胀导致 softmax 饱和、梯度过小。该机制允许模型在一个时间步内显式聚合序列中任意位置的信息，从而捕捉长程依赖关系。

\subsubsection{KV Cache机制的数学原理与复杂度分析}

LLM 的文本生成过程本质上是自回归的：模型在生成第$N$个 token 时，需要利用此前$N-1$个token的上下文信息。在自注意力模块中，对长度为$N$的序列执行一次前向计算，其单步复杂度为$O(N^{2})$。因此，生成长度为$N$的序列的总计算复杂度为：
\begin{equation}
T_{\mathrm{total}}^{\mathrm{naive}}=\sum_{t=1}^{N}O(t^{2})=O\!\left(\frac{N(N+1)(2N+1)}{6}\right)\approx O(N^{3})
\end{equation}

尽管现代推理框架通常不会采用完全重复前向的实现，但它直观地说明了自回归生成模式中重复计算的来源：每一步都在重新计算历史token对应的 Key/Value以及它们之间的注意力关系。而KV Cache 的基本思想就是缓存这些历史值并在后续解码中复用。由于模型参数在推理阶段固定，历史Token $x_i\ (i<t)$的$K_i$、$V_i$一旦计算便不再改变，因此可将其存储在GPU显存中。在第$t$步生成时，系统仅需计算当前 Token $x_t$的$k_t$和$v_t$，并将其追加到缓存末尾：
\begin{equation}
...
随后，注意力计算仅涉及当前 Query $q_t$与历史 $K_{\mathrm{cache}}$、$V_{\mathrm{cache}}$的交互，即计算$q_t K_{\mathrm{cache}}^{\top}$并加权$V_{\mathrm{cache}}$。因此，KV Cache 将解码阶段的单步复杂度由$O(t^{2})$ 降低为 $O(t)$，其仍然会随着上下文长度线性增长，生成$N$个 token的累计复杂度约为$\sum_{t=1}^{N}O(t)=O(N^{2})$，相应代价是缓存的显存占用随序列长度线性增长。
...
(a) System A: 原生推理(No Cache)\qquad (b) System B: 增量推理(KV Cache)
\caption*{\textbf{图2\quad 基于 KV Cache 的推理机制示意}\\
Fig.~2\quad Schematic of the KV Cache-based inference mechanism}
\end{figure}
...
其中$S$为替换（Substitution）、$D$为删除（Deletion）、$I$为插入（Insertion）的错误数量，$N_{\mathrm{ref}}$为参考文本的总词数。通常以百分比形式呈现，此时可再乘以100\%。对于中文数据集，本研究主要考察字符错误率（Character Error Rate, CER），其计算逻辑与WER一致。

\section{系统设计}

\subsection{总体架构设计}

\subsubsection{目标与逻辑拓扑}

本系统的核心设计目标是实现从传统的“全量接收-全量处理”范式向“增量接收-流式处理”范式的转移，从而在理论边界上最小化首字延迟。系统的逻辑拓扑如图 3 所示。图中展示了基于多线程生产者-消费者队列的流式并行数据流向。音频以固定时长的PCM块持续进入分段模块，Silero VAD在累计缓冲区上进行活动检测并输出语音段\upcite{ref13}；ASR 模块将若干语音段拼接后调用Whisper进行转录，并按前缀/后缀上下文策略输出稳定文本片段；文本片段经线程安全队列传递给 LLM 模块，持续更新 KV Cache 并在收到终止标记后启动生成，从而尽可能将 LLM 的预填充计算前移并与音频输入过程重叠。

整体架构采用分段-转录-预填充-生成的流水线并行方式：当上游模块产生新的音频或文本段时，下游模块即可立即启动计算，无需等待整段输入结束。系统由以下三个子系统组成：负责语音活动检测与分段的Streaming Audio Segmenter、负责上下文拼接并保证输出稳定的Context-Aware ASR Engine ，以及负责增量预填充与最终生成的Incremental LLM Inference Service。其中 LLM 部分的KV Cache属于Transformer解码器推理阶段的通用优化思路，本文直接复用推理框架内置的\texttt{past\_key\_values}机制完成状态复用。

\subsubsection{关键工程实现}
...
\caption*{\textbf{图3\quad 流式并行架构逻辑拓扑图}\\
Fig.~3\quad Logical topology of the streaming parallel architecture}
\end{figure}

而状态管理系统则是采用了有状态与无状态的混合设计。分段模块通过 StreamState维护累计音频缓冲、语音段起止时间与段编号等信息；ASR侧通过ASRCache管理等待加入的段队列与当前转录窗口段队列，并使用 total\_duration 与处理标记避免并发竞争；LLM侧则以 KVCache持久化\texttt{past\_key\_values}与\texttt{attention\_mask}、以及预填充阶段产生的最后一步\texttt{logits}，使得终止标记到达后可以直接进入生成阶段。上述状态对象共同保证了跨片段上下文的一致性，并为后续 3.2 与 3.3 的算法描述提供了相应的依据。

\subsection{流式ASR上下文管理}
...
\subsubsection{动态 VAD 分段策略}

系统采用Silero VAD模型对连续PCM音频流进行在线分段。定义输入音频流为时间序列 $\{x_1,x_2,\dots,x_t\}$，分段器维护累计缓冲区$A_{\mathrm{accum}}$，并在每次接收新的音频块$c_t$后执行 $A_{\mathrm{accum}}\leftarrow A_{\mathrm{accum}}\oplus c_t$，其中$\oplus$表示拼接。当累计音频长度超过最小检测窗口后，系统调用 Silero VAD 进行语音活动检测；若检测到足够长的静音，且语音段长度超过最小语音阈值，则认为该语音段已闭合，并输出给下游 ASR。本文原型系统的实验配置为：音频块长度 500 ms、最小语音长度 2 s、最小静音时长 300 ms。上述参数主要基于实时性与分段稳定性的折中选择。500 ms 块长能够提供较细的在线检测粒度，同时避免过短块长带来的频繁 VAD 调用和线程调度开销；2 s 最小语音长度用于保证每次提交给 Whisper 的语音片段具有足够上下文，减少过短片段导致的识别不稳定；300 ms 静音阈值则用于区分句内短暂停顿与真实语音结束，避免系统过早截断语音段。从参数敏感性看，较小的音频块长或静音阈值有助于降低端点检测等待时间，但可能增加误分段和边界处识别抖动；较大的块长、最小语音长度或静音阈值则可提升分段稳定性，但会增加端点后等待延迟。因此，本文在所有对比实验中固定上述 VAD 参数，以保证不同系统配置之间的公平比较。

\subsubsection{基于滑动窗口的上下文感知}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\linewidth]{media/media/image50.pdf}
\caption*{\textbf{图4\quad 上下文感知 ASR 滑动窗口机制示意图}\\
Fig.~4\quad Schematic of the context-aware ASR sliding-window mechanism}
\end{figure}

为了解决流式识别中的边界效应，ASR 引擎引入了包含前缀上下文的滑动窗口机制。图4详细展示了时间步$t$时ASR模型的输入构成。窗口被划分为三个逻辑区域：前缀上下文为历史已确定的冻结区域，提供必要的声学历史信息；当前片段为待识别的目标推理区域；后缀缓冲则包含未来音频信息以防止截断效应。系统通过这种重叠切分策略，确保了流式识别结果的连贯性。

首先，执行窗口更新。设$Q_t$为时刻$t$的音频段队列。当新的音频段到达时，系统先将其加入等待队列，并在转录线程中批量并入主队列得到$Q_t$；当队列累计时长满足触发阈值$\tau_{\mathrm{asr}}$，且队列长度满足要求，具体如公式（7）所示。
\begin{equation}
\left|Q_{\mathrm{temp}}\right|\geq N_{\mathrm{prefix}}+1+N_{\mathrm{suffix}}
\end{equation}

其中$\left|Q_{\mathrm{temp}}\right|$为当前临时队列长度，$N_{\mathrm{prefix}}$与$N_{\mathrm{suffix}}$分别表示系统配置需要的前置队列与后续队列片段的长度。当满足条件或者收到了最后一个语音段时，ASR模型M对临时队列中的所有音频段进行拼接后执行一次文本转录，生成原始转录结果。在工程实现中，往往还会增加一个最短识别阈值进行控制，用于避免过短窗口造成的频繁调用与不稳定输出。

其次，进行确定性文本提取。鉴于Whisper等ASR模型在处理流式输入的末尾时往往存在“闪烁”现象：即随着新音频的输入，原本输出的末尾词汇可能发生变化，因此系统在输出侧加入“后缀保护”段以增强输出文本的稳定性，系统仅输出前缀段与后缀段之间的稳定区域内的文本。为保证输出与音频段一一对应，本文实现利用Whisper自带的词级时间戳将原始转录结果中的词按结束时间映射回每个音频段的时间区间，从而得到段级候选文本；随后仅拼接稳定区域内段的文本形成本轮输出。在工程实现中，系统还对流式起止边界做了特殊处理：在流式起始轮时，前缀段尚未输出过，稳定区的起点可从0开始；在流式结束轮时，为避免遗留文本，系统会输出所有剩余段并完成收敛。最后，执行缓冲区状态迁移。在输出稳定区域文本后，窗口执行滑动以限制累计长度并维持声学连贯性：系统不会将队列全部清空，而是保留最后一个已输出段之前的$N_{\mathrm{prefix}}$个段作为下一轮的前缀上下文，同时继续保留尚未输出的后缀保护段。需要指出的是，稳定输出的代价并非仅增加一个分片的固定延迟，而是至少需要等待$N_{\mathrm{suffix}}$个后续段到达、并满足触发阈值 $\tau_{\mathrm{asr}}$。但该代价换来了输出文本在边界处的稳定性与准确性，使下游LLM能够稳定地进行增量预填充，从系统层面实现端到端延迟可控下降，同时保证输入的准确性。

\subsection{LLM KV Cache增量预填充策略}
...
为减少语音端点后的计算量，支撑流水线并行，System B在LLM部分引入了增量预填充：语音端点之前，使用已经产生的流式文本进行预处理，在Transformer解码器预填充时产生的\texttt{past\_key\_values}作为KV Cache的历史，保存在内存或GPU显存中，在新文本片段抵达时，利用保存的\texttt{past\_key\_values}，只对新增token做前向计算，再将计算后新的\texttt{past\_key\_values}替换旧值，在另一段新文本到达时重复上述过程。这样LLM的大部分增量预填充可以在用户说话期间完成。当终止标记到达时，系统只需处理末尾片段，马上开始生成回复token的计算工作，从而减少语音结束到首token输出的等待时间。

\subsubsection{核心算法实现}

增量推理的核心逻辑：首次调用时，系统在无缓存状态下一次性执行增量预填充，建立初始KV Cache；随后，每当上游传来一段新的ASR文本片段，系统便触发一次调用\texttt{cache\_prompt()}：在已有\texttt{past\_key\_values}的基础上做增量更新。当收到终止标记时，系统在提示文本末尾追加生成提示符（例如“assistant:”，用于告诉模型后续输出的首token即是智能助理回复），并复用预填充阶段最后一步的logits直接开始解码。为避免增量token的位置编码错位，实现里会显式构造\texttt{position\_ids}，同时将新增片段以不增加生成提示符的方式编码，确保缓存拼接可控。Algorithm 1给出了与该实现对应的增量更新过程。图5直观展示了显存内的状态变化。左侧蓝色区域表示历史上下文对应的缓存键值对，右侧绿色区域表示新增片段产生的键值对。增量预填充在新片段到达时复用历史\texttt{past\_key\_values}，仅对新增token执行前向并更新缓存，从而避免在增量场景下反复对全量提示文本进行预填充。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\linewidth]{media/media/image64.pdf}
\caption*{\textbf{图5\quad LLM KV Cache 增量更新机制示意图}\\
Fig.~5\quad Schematic of the LLM KV Cache incremental update mechanism}
\end{figure}

\subsubsection{复杂度分析}

为了说明增量预填充减少冗余计算的来源，考虑历史上下文长度为$N$、新增片段长度为$M$的一次更新：若在每次增量到达时都对全序列$x_{1:N+M}$重新执行一次预填充（不复用历史状态），则自注意力的主要开销近似为$O\!\left((N+M)^{2}\right)$。而在使用KV Cache的情况下，历史token的$K,V$已经缓存，系统仅需对新增token计算Query并与长度为$N+M$的Key/Value交互，主要开销可写为$O\!\left(M(N+M)\right)$。当$M\ll N$时，上式近似为$O(MN)$，体现为对新增片段的线性扩展。需要强调的是，若将所有增量片段累积到最终长度$L$，增量预填充的累计开销为
\begin{equation}
\sum_{j}O\!\left(M_j L_j\right)=O\!\left(L^{2}\right)
\end{equation}

其中$M_j$与$L_j$分别为第$j$个新增文本序列的长度与累积长度，$L$为完整序列长度。与离线一次性预填充同阶。本文策略的关键收益不在于改变复杂度阶数，而在于将预填充计算前移到用户发声期间，使语音结束端点之后需要完成的计算主要集中在最后一小段增量及线程同步开销，从而有效降低TTFT。与此同时，本文实现会在KV Cache中保留预填充阶段最后一步\texttt{logits}，使得终止标记到达后无需额外前向计算即可解码首 token，进一步减少首字阶段的常数开销。

\subsection{实验数据构建与处理管线}

\subsubsection{简述}

为了在受控环境下系统性验证流式架构的性能，需要有不断递增长度的对话数据，本研究构建了一套自动化的数据合成管线。该管线将 MultiWOZ（英文）\upcite{ref14}和 CrossWOZ（中文）\upcite{ref15}这两个标准多轮对话数据集转化为具有精确时长标注的逐渐递增的长语音测试数据，再通过TTS生成音频数据。

\subsubsection{累积对话生成逻辑}

\begin{table}[htbp]
\centering
\caption*{\textbf{表1\quad 对话数据累积方式}\\
Table 1\quad Dialogue Data Accumulation Strategy}
\begin{tabular}{clll}
\toprule
轮次 & 用户 & 客服 & 数据文本 \\
\midrule
...
3 & 要商务舱 & 好的，这就为您预订 & \begin{tabular}[t]{@{}l@{}}订一张机票，去哪里？，去北京，明天出发。\\ 要哪种舱位呢？要商务舱\end{tabular} \\
\bottomrule
\end{tabular}
\end{table}
...
其次是并发TTS音频合成。近年来神经语音合成的自然度已接近真人水平\upcite{ref16}，本研究集成了阿里巴巴提出的CosyVoice大模型语音合成服务\upcite{ref17}。相较于传统TTS，CosyVoice能够生成韵律更自然、情感更丰富的高保真语音波形，更贴近真实人声输入。在工程实现上，开发了批量处理模块，采用多线程异步请求机制，显著提升了大规模数据生成的效率。
...
\subsubsection{测试集分组定义}

\begin{table}[htbp]
\centering
\caption*{\textbf{表2\quad 音频时长分组}\\
Table 2\quad Audio Duration Groups}
\begin{tabular}{clll}
\toprule
组名 & 时长(s) & 典型场景 & 实验聚焦 \\
\midrule
Short & $T<5$ & 短指令 & 基准延迟验证 (Baseline Latency) \\
Medium & $5\leq T<15$ & 包含多轮意图的陈述 & 日常对话性能 \\
Long & $15\leq T<30$ & 复杂长难句/长段落 & 流式架构核心优势区间 \\
Very Long & $30\leq T<60$ & 长段论述说明 & 判断TTFT是否存在稳定上界 \\
Extra Long & $T\geq60$ & 极限压力测试 & 系统稳定性与显存边界 (Stability \& OOM) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption*{\textbf{表3\quad 不同语音时长分组下的 TTFT 统计}\\
Table 3\quad TTFT Statistics Across Speech Duration Groups}
...
表3清晰地给出了按语音时长分组的 TTFT 统计结果。非流式基线System A的TTFT随输入时长有明显增长。其中Short组均值为533.42ms，Long组上升到了1722.04ms，Very Long与Extra Long组进一步增长到3191.97ms与6753.43ms。这一现象和级联式处理范式的预期是相符的，因为ASR需等待语音生成完成之后才能进行全量解码，之后LLM才会对完整转录文本一次性进行全量预填充。在此期间，端点之后的计算链路几乎不会重叠，这就使得语音长度与TTFT基本是线性相关的关系。而流式系统 System B 在 Long及以上区间显示出近似常数的上界：Long、Very Long与Extra Long三组的 TTFT 均值分别为 1126.63ms、1099.16ms 与 1087.70ms，整体稳定在 1.1s 左右。相应的TTFT在 Long、Very Long、Extra Long这三个分组上，相对于基线系统分别压缩了34.6\%、65.6\%与83.9\%，其中Extra Long组平均绝对延迟降低了5.67s，明显减轻了长语音交互时的静默等待。需要说明的是，这里的常数是指主导项由随时长累积的全量计算耗时，转变为最后若干分片的尾部处理与调度开销之和。在音频包时长500 ms与计算资源不变的前提下，TTFT 不再随输入时长的增长而继续显著增加。

值得注意的是，在 Short 与 Medium 组中，System B 相较于 System A 分别出现了 114.75 ms 和 36.11 ms 的延迟增加。本文将延迟变化量定义为$\Delta \mathrm{TTFT}=\mathrm{TTFT}_{\mathrm{SystemB}}-\mathrm{TTFT}_{\mathrm{SystemA}}$，因此$\Delta \mathrm{TTFT}>0$表示流式方案的 TTFT 高于非流式基线，即存在负向收益；$\Delta \mathrm{TTFT}<0$则表示延迟降低。在短语音场景下，分片处理、缓存维护和增量提示构造等流式架构的固定开销难以被并行计算收益抵消，同时较短的语音和文本输入尚未充分触发长上下文预填充的计算瓶颈。该结果表明，在工程落地时可根据输入时长进行自适应路径选择：对预测时长较短的输入优先采用传统一次性处理链路，而将流式并行机制主要用于音频时长$T\geq 15\,\mathrm{s}$的长语音场景，以获得更优的整体交互体验。

\subsection{实验二：消融实验}

\subsubsection{实验设置}

为了量化流式ASR与LLM增量预填充两项机制的相对贡献，本文挑选了Long及以上长度的各50个样本开展消融实验。按照前面章节描述的分组，本文选择了Long（15--30s）、Very Long（30--60s）与Extra Long（$\geq$60s）三个分组，因为在该长度区间能比较好地体现流式改造的收益。对比配置设置分为三类：Baseline（System A，ASR 与 LLM 均为非流式串行执行）、Streaming ASR Only（仅 ASR 流式化，LLM在等待全部文本到齐后再启动推理）以及Full Streaming（在 Streaming ASR Only 的基础上进一步启用LLM增量预填充，即上文中的 System B）。
...
\begin{table}[htbp]
\centering
\caption*{\textbf{表4\quad 核心模块消融实验结果}\\
Table 4\quad Ablation Results of Core Modules}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{ccccccc}
\toprule
组名 & \begin{tabular}{@{}c@{}}平均\\时长(s)\end{tabular} & \begin{tabular}{@{}c@{}}Baseline\\TTFT(ms)\end{tabular} & \begin{tabular}{@{}c@{}}Streaming ASR\\Only TTFT(ms)\end{tabular} & \begin{tabular}{@{}c@{}}Full Streaming\\TTFT(ms)\end{tabular} & \begin{tabular}{@{}c@{}}ASR\\improvement(ms)\end{tabular} & \begin{tabular}{@{}c@{}}KV Cache\\improvement(ms)\end{tabular} \\
\midrule
...
\subsubsection{贡献度量化分析}

实验结果如表4所示，在实验平台硬件及所用模型条件下，ASR流式化是缩短TTFT的主因。Streaming ASR Only与Baseline相比，在Long、Very Long与Extra Long三组长度下分别减少了634.70ms、2129.56ms与5289.63ms，对应降幅为37.4\%、64.5\%与81.1\%。这表明在传统串行链路中，语音结束端点后的主要耗时来自于ASR对整个音频的处理。而流式ASR把大部分计算提前到与语音生成的时间重叠，因此显著压缩了端点后的计算量。增量预填充在LLM方面表现出显著的长度依赖特性。在Long组，Full Streaming相比Streaming ASR Only的TTFT反而增加了19.99ms（KV增益为-19.99ms），这说明当上下文还没足够长的时候，增量调用、缓存管理以及同步这些额外开销或许会超出其节省的一次性预填充计算。而在Very Long与Extra Long组，KV增益分别为16.96ms与114.20ms，且在Extra Long组，KV Cache相对整体的收益占比达到约9.3\%（114.20/1228.77），这表明，随着输入文本与上下文长度增长，LLM预填充的收益愈加显著。

综合两项优化机制，Full Streaming将TTFT均值控制在1.1s（Long 1084.17ms、Very Long 1154.06ms、Extra Long 1114.57ms）左右，相较于Baseline分别缩减了约36.2\%、65.0\%与82.9\%。该结果与实验总体趋势一致，流式并行能明显降低级联系统在输入时长增加时的等待时间，不过KV Cache预填充的边际收益还是受工程实现和硬件并发与算力的影响。消融实验表明，在实验平台的硬件条件下，运行Whisper模型已显吃力。而Qwen2-7B-Instruct的体量在LLM领域算是小型模型，其推理计算量远小于高性能模型（参数量往往在数百到上千B），因此实验平台的算力足以应对。如果换为主流SOTA模型，相信流式架构在KV Cache增量预填充上的收益将会更加明显。另外值得注意的是，三组递增长度的音频实验结果显示，只使用流式ASR和一次性预填充时，耗时还是逐渐递增的；而在全流式（使用了KV Cache）的结果中，三组音频的处理时长并没有递增的关系。这表示在实验环境下，整个转录文本的平方阶复杂度已经超出了硬件并行计算能力的上限，需要花费更长的时间来计算注意力。而在使用KV Cache的情况下，尾部计算复杂度相对于整个文本接近线性阶，并未达到硬件的并行处理上限，因此即使长度增加，处理时长也并未增加，说明KV Cache增量预填充机制确实能在长语音场景下显著降低计算成本。

\subsection{实验三：准确率与质量边界}

\subsubsection{实验说明与评估方法}
...
\subsubsection{精度-延迟分析}

\begin{table}[htbp]
\centering
\caption*{\textbf{表5\quad 不同上下文窗口配置下的准确率与耗时对比}\\
Table 5\quad Accuracy and Processing Time Comparison Under Different Context Window Configurations}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{cccccc}
\toprule
上下文配置 & \begin{tabular}{@{}c@{}}multiwoz\\wer\_mean\end{tabular} & \begin{tabular}{@{}c@{}}multiwoz\\cer\_mean\end{tabular} & \begin{tabular}{@{}c@{}}crosswoz\\wer\_mean\end{tabular} & \begin{tabular}{@{}c@{}}crosswoz\\cer\_mean\end{tabular} & ASR耗时(ms) \\
\midrule
...
表5展示了不同上下文窗口配置下的准确率与尾部耗时对比结果。可以看出，增加前后缀（pre1\&suf1）对中文句尾稳定性的提升更加明显：中文数据集CrossWOZ的WER/CER 分别由默认配置的 0.0118/0.0292 下降到了 0.0083/0.0203。由于处理片段长度增加，也导致了语音结束端点后的尾部耗时上升，使\texttt{asr\_time\_ms}由1224.96ms增加到了1327.48ms（+102.52ms），这与后缀窗口需要等待额外音频片段并完成更多解码计算的机制预期相符。而对于英文数据集的 MultiWOZ，suffix的增加使 CER 从 0.0221 降低到了 0.0196，不过 WER 稍微升高到了 0.0895，说明在以词为单位的评价标准下，后缀窗口对齐和分词的影响可能更敏感了。这一现象有必要通过真实录音及更大样本数据做进一步验证。当完全去除额外上下文（pre0\&suf0）时，系统获得了更低的耗时（\texttt{asr\_time\_ms}下降至 1086.16ms，相比默认耗时减少 138.80ms），中文误差在预料之内明显变大了：CrossWOZ 的 CER 上升到了 0.0500，WER 也上升到了 0.0210。该结果显示，prefix给出的那些历史上下文在流式识别里发挥了重要的约束作用，能给音频-语言模型的局部决策带来更强纠正效果，从而抑制长句里逐步累积的误差。
...
最后从端到端延迟预算看，实验一与实验二显示 Long及以上语音的 TTFT 已可稳定压缩至约 1.1s，而表5中不同上下文配置带来的 0.1--0.2s 尾部耗时已占去可观比例，因此需按场景进行取舍：对识别正确性更敏感的中文任务，可启用 pre1\&suf1 以降低句尾错误；pre0\&suf0 则作为极低延迟模式，或用于英文对话。考虑到对话式 LLM 在语义层面通常对噪声具有一定鲁棒性，转录文本的小误差预计对 LLM 的推理不会造成明显影响。后续可引入语义一致性与任务完成率等指标，直接量化转写误差造成的语义漂移程度。

\section{总结与展望}

\subsection{全文总结}

本文围绕级联式语音对话系统在长语音输入下端点后等待时间过长的问题，设计并实现了一套基于流水线并行的低延迟语音对话架构。该架构以流式计算和状态增量为设计主线，将传统串行链路中的ASR全量转录与LLM一次性预填充改造为可重叠执行的流式处理过程：在ASR侧，通过VAD分段、滑动窗口和前缀/后缀上下文，把偏向离线处理的 Whisper 封装为可输出稳定增量文本的流式模块；在 LLM 侧，利用\texttt{past\_key\_values}对新增文本片段进行增量预填充，并复用预填充阶段最后一步\texttt{logits}启动首token解码。需要指出的是，该策略并不改变注意力计算的复杂度阶数，而是把原本集中在语音结束端点之后的预填充计算提前到用户说话过程中，从而减少端点后的关键路径耗时。

实验结果验证了该架构的有效性。在从 MultiWOZ 与 CrossWOZ 构建的长语音数据集上，本文系统比传统非流式基线在 15 秒以上的长语音分组中具有稳定的优势，其将TTFT保持在了一个稳定的范围内。同时通过滑动窗口上下文维持了语音转录文本的准确性，WER/CER处于可接受范围内。虽然在Long及以上分组的TTFT仍约为1.1s，与人类对话中约200ms的对话间隙仍有一定差距，但与传统串行方式相比，其最大的优势在于长语音场景下的TTFT在达到一定数值后不再随着语音时长增长\upcite{ref21}。未来借助更高效的推理硬件、更低延迟的ASR模型及进一步调度优化，继续压缩这段端点后等待时间，即可实现任意长度语音均保持与人类对话间隙一致的响应延迟。

\subsection{研究局限性}

尽管本文降低了长语音场景下的端点后等待，但系统仍受级联架构约束。首先，当前原型仍遵循用户发言-系统处理-系统回复的半双工话轮模式，尚不能处理自然对话中的重叠说话与打断\upcite{ref22}。其次，ASR 与LLM之间仍以文本作为唯一接口，语调、情绪、停顿等副语言信息会在转写过程中丢失。再次，本文测试音频主要由TTS合成，真实噪声、口音和麦克风条件下的鲁棒性仍需进一步验证。
