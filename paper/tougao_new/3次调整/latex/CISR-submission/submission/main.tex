...
\begin{abstract}
Cascaded voice dialogue systems (ASR--LLM--TTS) are the dominant spoken-interface backbone of modern intelligent systems, yet they suffer post-utterance waiting time that grows with utterance length. This work aims to keep the time to first token (TTFT) roughly constant for long-form speech while preserving the modularity of the cascaded architecture. A pipeline-parallel streaming architecture overlaps speech recognition with language-model prefilling. Voice activity detection enables online segmentation, and an adaptive sliding window with dynamic buffering leverages Whisper timestamp alignment to commit stable transcripts under a prefix--suffix context and a local-consistency constraint. Incremental prefilling with a key--value cache updates only newly arrived text. The system is evaluated on 1,132 progressively lengthening utterances synthesized from MultiWOZ and CrossWOZ. Mean TTFT remains around 1.1~s in long-utterance groups, achieving 34.6\%--83.9\% reductions over a non-streaming baseline and a 5.67~s average reduction in the longest group, while transcription error rates stay within an acceptable range. Streaming pipeline parallelism and state incrementalization thus effectively reduce long-utterance interaction latency in modular cascaded systems.
\end{abstract}
...
In most conventional cascaded implementations, several seconds may elapse between the end of the user's speech and the system's spoken reply. In long-form scenarios in particular, ASR cannot start until the user finishes the entire utterance, and the LLM's prefill time grows with the length of the input text. As intelligent systems are increasingly deployed in scenarios that demand natural, low-latency spoken interaction---such as voice-driven service robots and conversational assistance---the responsiveness of the voice interface itself becomes a usability bottleneck. How to retain the modularity of the cascaded architecture while using streaming parallelism to shorten the ``dead time'' between modules---so that the system responds the moment the user stops speaking---is therefore of both academic and engineering interest to the intelligent systems community.
...
First-token latency is the core measure of conversational fluency. Fig.~\ref{fig:trend} plots the TTFT trajectories of the non-streaming baseline and our streaming system across input durations.
