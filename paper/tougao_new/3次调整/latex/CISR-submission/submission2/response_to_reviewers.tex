...
\changes We added the construction and augmentation protocol in Section IV-E (p.~8), added Section V-D (pp.~9--10) and Table~VI (p.~10), and expanded Limitations (p.~11). New citations cover LibriSpeech, AISHELL-1, and MUSAN.
...
\changes We added separate TTFT/TTFA definitions in the Introduction and Section III-C (pp.~1 and 4), added Section V-G (p.~11), and added Table~VIII with total and component distributions (p.~12). Limitations on p.~11 now state the localhost and non-distributed boundary.
...
\changes We replaced Fig.~6 and expanded Tables III--V (pp.~9--10), added the repeated-measurement paragraph in Section V-B (p.~9), added the statistical protocol in Section III-C (p.~4), and disclosed the exclusive-GPU boundary in Limitations (p.~11).
...
\changes We added the baseline method and citations in Sections II-D and V-E (pp.~2 and 10) and Table~VII (p.~11). Table~IV and the ablation discussion use the reproducible 498-sample values (p.~10). Platform notes explicitly separate original-platform Table~IV from second-platform Table~VII.
...
\changes We added the commit contract and tokenizer-seam definition in Section IV-B (p.~6), added Section V-F (pp.~10--11), cited BGE-M3 and the exact judge service documentation, and expanded Limitations and Future Work (p.~11).
...
\item We corrected the methods description to distinguish the 2.0-s ASR recognition trigger from the Silero minimum-speech setting and to report the legacy and unified-TTFA VAD configurations separately; no experiment or result was changed.
\item We replaced secondary or mismatched citations with primary sources for Conformer, PagedAttention, FlashAttention, StreamingLLM, Mini-Omni, Moshi, LLaMA-Omni, and Whisper, and corrected the associated BibTeX metadata.
\end{itemize}
