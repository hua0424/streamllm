...
PCM_BYTES_PER_SAMPLE = 2  # 16bit mono


def probe_service(base_url: str, timeout: int = 10) -> bool:
    """服务探活：用短文本发一次请求，确认服务可用"""
    try:
        response = requests.post(
            f"{base_url.rstrip('/')}/inference_sft",
            data={"tts_text": "探活", "spk_id": SPK_ID, "stream": True, "speed": SPEED},
            timeout=timeout,
            stream=True,
        )
        ...
        logger.info(f"服务探活{'通过' if ok else '失败（无音频数据）'}: {base_url}")
        return ok
    except Exception as e:
        logger.error(f"服务不可达 {base_url}: {e}")
        return False


def measure_one(base_url: str, text: str, timeout: int = 120) -> dict:
    """
    ...
    try:
        ...
        response = requests.post(
            f"{base_url.rstrip('/')}/inference_sft",
            data={"tts_text": text, "spk_id": SPK_ID, "stream": True, "speed": SPEED},
            timeout=timeout,
            stream=True,
        )
        if response.status_code != 200:
        ...
        audio_sec = total_bytes / (PCM_SAMPLE_RATE * PCM_BYTES_PER_SAMPLE)
...
def load_from_e4(e4_dir: Path, n_zh: int, n_en: int, seed: int = 42) -> list:
    ...
    for r in data["results"]:
        ...
        if text.strip() and lang in pool:
            pool[lang].append({"sample_id": r["sample_id"], "text": text.strip(), "language": lang})
...
def main():
    ...
    parser.add_argument('--url', type=str, default='http://host.docker.internal:20401',
                        help='TTS 服务地址')
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(["sample_id", "language", "n_chars", "ttfc_ms",
                         "total_ms", "audio_sec", "rtf", "error"])
        for i, item in enumerate(items):
            m = measure_one(args.url, item["text"], timeout=args.timeout)
            writer.writerow([
