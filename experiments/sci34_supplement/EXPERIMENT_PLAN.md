...
- 统计：事件数、Wilson CI、exact McNemar、dialogue-cluster bootstrap 95% CI；分位置为次分析。
...
### S-P1：headless 异步控制路径

**RQ：** 无声卡、真实墙钟节拍的播放器收到 stop 后，软件停播确认、时间轴反查和 GPU 状态修正的延迟分布如何？

- 上下文：512、2048、8192。
- 位置：25%、50%、75%。
- repeats：20，共 180 events。
- 主播放条件：24 kHz、20 ms block、0.8 s synthetic duration、4 fragments。
- 指标：stop ack、leaked samples、lookup、crop-only、role-only、stop→crop、stop→role、wakeup error。
- player 使用绝对 deadline 和可中断 `Event.wait()`，TTS chunk 与播放 block 分离。
...
- 同一个 pair 的两条件共享 trajectory ID 和 target delta。
...
- stop ack 后 sample counter 不再推进。
- timeline 返回合法 token endpoint。
- 180 条事件完整落盘。
- 不对真实声卡或声学停止做任何主张。
