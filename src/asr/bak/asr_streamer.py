...
class ASRCache:
    ...
    """
    segment_queue: List["AudioSegment"] = field(default_factory=list)
...
class StreamingASRProcessor:
    ...
    def add_audio_chunk(self, cache: ASRCache, audio_chunk: np.ndarray, is_stream_finished: bool = False) -> Tuple[ASRCache, str]:
        """
        添加音频块并返回新的转录文本，输出文本可能为空
        @param cache ASR缓存对象
        @param audio_chunk 音频信号转换为张量
        @param is_stream_finished 流式输入是否结束
        @return str 新输出的文本
        ...
        logger.debug(f"添加新音频块，是否结束: {is_stream_finished}")
        if is_stream_finished:
            logger.debug("🎉 检测到最后一个音频块，流结束")
